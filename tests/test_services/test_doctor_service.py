...
class TestDoctorSerivce(TestCase):
    ...
    def test_add_doctor_already_exists(self):
        self.doctor_repo.doctor_exists.return_value = True

        result = self.doctor_s.add_doctor(self.doctor) 

        self.doctor_repo.doctor_exists.assert_called_once_with(self.doctor)
        self.assertEqual(result,{'success':False,'message':"Doctor Already Exists."})


    def test_add_doctor_success(self):
        self.doctor_repo.doctor_exists.return_value = False
        self.doctor_repo.add_doctor.return_value = 1

        result = self.doctor_s.add_doctor(self.doctor)

        self.doctor_repo.doctor_exists.assert_called_once_with(self.doctor)
        self.doctor_repo.add_doctor.assert_called_once_with(self.doctor)
        self.assertEqual(result, {'success':True,'message':"Doctor Added Successfully."})

    def test_add_doctor_failed(self):
        self.doctor_repo.doctor_exists.return_value = False
        self.doctor_repo.add_doctor.return_value = 0
    ...
    def test_add_doctor_exception_occurs(self,mocked_dblogger):
        self.doctor_repo.doctor_exists.return_value = False
        self.doctor_repo.add_doctor.side_effect = sqlite3.IntegrityError("demo error")
